...
生成物:
- **週報**（管理者向け。**ファイルには出力せず、パスワード保護されたダッシュボードにのみ保存**されます。リポジトリにも実行ログにも本文は残りません）
...
実行すると週報を生成し、指標CSV・グラフ・暗号化ダッシュボードデータ（`docs/data.enc`）をリポジトリへコミットします。
**週報本文はコミットにもActionsの実行ログ・サマリー・Artifactにも一切残さず、ダッシュボードにのみ保存**します。
...
週報で言及される上位チャンネル）・チャンネル/スレッドの盛り上がり Top5・イベント一覧・**週報本文**
（Markdownレンダリング表示）を表示し、「週報を生成」ボタンから GitHub Actions の実行画面へ遷移できます。

### パスワード保護

ダッシュボードのデータ（**週報本文を含む**）は **AES-256-GCM で暗号化して配信**され、閲覧時にパスワードの入力が必要です。

- **Secrets に `DASHBOARD_PASSWORD` を登録**してください（未設定だとワークフローがエラーで停止します）
- ワークフロー実行時にデータを暗号化した `docs/data.enc` だけを公開し、平文の `docs/data.json` は生成しません（存在すれば削除）
- ブラウザ側は WebCrypto（PBKDF2-SHA256 310,000回 + AES-GCM）で復号します。誤ったパスワードでは復号できません
- 入力したパスワードは同一タブ内でのみ記憶されます（タブを閉じると再入力）
- 週報のMarkdownは [marked](https://github.com/markedjs/marked) でHTML化した後、[DOMPurify](https://github.com/cure53/DOMPurify) でサニタイズしてから表示します（Discordメッセージ内容がAIプロンプト経由で混入する可能性があるため、悪意あるHTML/スクリプトを除去）
- `--skip-report`（指標のみ更新）実行時は、前回生成された週報を暗号化データ内で引き継ぎ、ダッシュボードから消えないようにします

**制限事項**:
- 全員で1つの共有パスワードです（ユーザー別認証ではありません）。パスワードを変更したら Secrets を更新して再実行してください
- ページのHTML/JS（レイアウト）自体は公開のままですが、データ（週報本文含む）は暗号化されており含まれません
- リポジトリ自体が Public の場合、リポジトリ経由でCSV等（集計値のみ、週報本文は含まない）が見えてしまうため、**リポジトリは Private を前提**とします
...
グラフ描画には Chart.js、週報のMarkdown表示には marked + DOMPurify を `docs/vendor/` に同梱しており、外部CDNに依存しません。
